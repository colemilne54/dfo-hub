            ...
            {(!this.state || !this.state.edit) && this.props && this.props.stakingData && this.props.stakingData.map(element => {
                ...
                lis.push(...element.tiers.map(it => <li key={it.blockNumber} className="TheDappInfoAll TheDappInfoSub KingJulianAlwaysWatchingYou">
                    ...
                    <section className="TheDappInfo1 TheDappInfoYY">
                        ...
                        {element.active && element.running && <p className="DFOHostingTitle DFOHostingTitleTTT TheDappInfoX">You'are be able to open Liquidity Mining positions{element.endBlock && <span> until the block n. <b><a target="_blank" href={window.getNetworkElement("etherscanURL") + "block/" + element.endBlock}>{element.endBlock}</a></b></span>}</p>}
                        {element.active && element.terminated && <p className="DFOHostingTitle DFOHostingTitleTTT TheDappInfoX">This Liquidity Mining Contract is closed until the block <b><a target="_blank" href={window.getNetworkElement("etherscanURL") + "block/" + element.endBlock}>{element.endBlock}</a></b>, you can only redeem for opened positions.</p>}
